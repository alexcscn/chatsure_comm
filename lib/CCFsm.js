...
const CCFsm = function () {
    ...
    self.SetMedia_1 = function (ucARx, ucATx, ucVRx, ucVTx, idLocalV, idPeerV, SrvType) {
        ...
        self.PeerConn.createOffer().then(self.onCreateOfferSuccess, self.onCreateOfferError);
    ...
    self.SetMedia = function (ucARx, ucATx, ucVRx, ucVTx, idLocalV, idPeerV, SrvType) {
        ...
        if (true == m_IDT_INST.bIsIe) {
            ...
            if (null == self.LocalStream) {
                ...
                let bAudio, bVideo;
                if (1 == ucARx || 1 == ucATx) bAudio = true;else bAudio = false;
                if (1 == ucVRx || 1 == ucVTx) bVideo = true;else bVideo = false;
                ...
                if (true == bAudio) {
                    if (true == bVideo) {
                        navigator.mediaDevices.getUserMedia({ audio: audioConst, video: videoConst }).then(self.GotStream);
                        //.catch(function(e) { alert("getUserMedia() error: " + e.name); });混淆不通过!!!!!!!!
                    } else {
                        navigator.mediaDevices.getUserMedia({ audio: audioConst, video: false }).then(self.GotStream);
                        //.catch(function(e) { alert("getUserMedia() error: " + e.name); });混淆不通过!!!!!!!!
                    }
                } else {
                    if (true == bVideo) {
                        navigator.mediaDevices.getUserMedia({ audio: false, video: videoConst }).then(self.GotStream);
                        //.catch(function(e) { alert("getUserMedia() error: " + e.name); });混淆不通过!!!!!!!!
                    } else {
                        //不应该到这里
                        navigator.mediaDevices.getUserMedia({ audio: false, video: false }).then(self.GotStream);
                        //.catch(function(e) { alert("getUserMedia() error: " + e.name); });混淆不通过!!!!!!!!
                    }
                }
            } else {
    ...
    self.SdpFilter = function (str, step, telEventId, H264Id) {
        ...
        if (-1 != str.indexOf('a=extmap:')) return false;
    ...
    self.FormatSdp = function (sdp, ucARx, ucATx, ucVRx, ucVTx, SrvType, bIsGCall) {
        let telEventId = 126;
        let H264Id = 100;
        if (ucARx || ucATx) telEventId = -1;
        if (ucVRx || ucVTx) H264Id = -1;
        ...
        for (i = 0; i < strArray.length - 1; i++) {
            if (-1 == telEventId) {
                ...
                if (-1 == strArray[i].indexOf('telephone-event/8000')) continue;
            ...
            if (-1 == H264Id) {
                ...
                if (-1 == strArray[i].indexOf('profile-level-id=42e01f')) continue;
                H264Id = parseInt(strArray[i].substring(7));
            }

            if (-1 != telEventId && -1 != H264Id) break;
        }
        ...
        for (i = 0; i < strArray.length - 1; i++) {
            ...
            if (false == self.SdpFilter(strArray[i], step, telEventId, H264Id)) continue;
            ...
            switch (step) {
                case 1:
                    if (IDT.SRV_TYPE_CONF == SrvType || IDT.SRV_TYPE_CONF_JOIN == SrvType) {
                        ...
                        if (1 == ucARx && 1 == ucATx) strDst += 'a=sendrecv\r\n';else if (1 == ucARx && 0 == ucATx) strDst += 'a=recvonly\r\n';else if (0 == ucARx && 1 == ucATx) strDst += 'a=sendonly\r\n';
                    }
                    break;
                case 2:
                    if (IDT.SRV_TYPE_CONF == SrvType || IDT.SRV_TYPE_CONF_JOIN == SrvType) {
                        if (false == bIsGCall) strDst += 'a=sendrecv\r\n';
                    } else {
                        if (1 == ucVRx && 1 == ucVTx) strDst += 'a=sendrecv\r\n';else if (1 == ucVRx && 0 == ucVTx) strDst += 'a=recvonly\r\n';else if (0 == ucVRx && 1 == ucVTx) strDst += 'a=sendonly\r\n';
                    }
                    break;
                default:
                    break;
    ...
    self.onIceStateChange = function (that, event) {
        if (null == self.PeerConn) CommLogger.info("CCFsm[", self.uiMyId, "]", "onIceLocalStateChange:", event);else CommLogger.info("CCFsm[", self.uiMyId, "]", "onIceLocalStateChange:", event, self.PeerConn.iceConnectionState);
    };
    ...
    self.onIceCandidate = function (that, event) {
        ...
        if (null == event.candidate) return;
        return;
    ...
    self.onCreateOfferSuccess = function (sdp) {
        ...
        self.PeerConn.setLocalDescription(sdp).then(function () {
            self.onSetLocalSuccess(sdp);
        }, self.onSetLocalError);
    ...
    self.SdpGetMyAttrFromPeer = function (sdp, attr) {
        ...
        if (1 == sdp.ARx) attr.ATx = 1;
        if (1 == sdp.ATx) attr.ARx = 1;
        if (1 == sdp.VRx) attr.VTx = 1;
        if (1 == sdp.VTx) attr.VRx = 1;
        return 0;
    };
    //--------------------------------------------------------------------------------
    //      启动呼出
    ...
    self.CallMakeOut = function (UserCtx, idLocalV, idPeerV, ucARx, ucATx, ucVRx, ucVTx, strMyNum, strPeerNum, SrvType, strPwd, ucCallOut, ucDelG, ucUserMark) {
        CommLogger.info("CCFsm[", self.uiMyId, "]", "CallMakeOut", strMyNum, strPeerNum, SrvType, ucARx, ucATx, ucVRx, ucVTx, UserCtx, strPwd, ucCallOut, ucDelG, idLocalV, idPeerV);
    ...
    self.RecvSetupAckProc = function (RecvMsg) {
        ...
        if (self.State != self.CC_WAIT_SETUPACK) return -1;
    ...
    self.SetPeerSdpFunc = function (sdp) {
        ...
        if (true == self.SetPeerSdp) return 0;
        ...
        if (true == m_IDT_INST.bIsIe) {
            if (null != self.idPeerV) self.idPeerV.setRemoteDescription(self.PeerSdp);
        } else {
            self.PeerConn.setRemoteDescription(self.PeerSdp).then(self.onSetRemoteSuccess, self.onSetRemoteError);
        }
    ...
    self.RecvConnProc = function (RecvMsg) {
        // 检查状态
        if (!(self.CC_WAIT_CONN == self.State || self.CC_RUNNING == self.State)) return -1;
    ...
    self.RecvSetupProc = function (RecvMsg) {
        ...
        self.SrvType = RecvMsg.MsgBody.SrvType;
        if (IDT.SRV_TYPE_NONE == self.SrvType) self.SrvType = IDT.SRV_TYPE_BASIC_CALL;
        //修正SrvType??????
        switch (self.SrvType) {
            case IDT.SRV_TYPE_CONF:
            case IDT.SRV_TYPE_CONF_JOIN:
                self.SrvType = IDT.SRV_TYPE_CONF;
                if (null != RecvMsg.MsgBody.OrigCalledName) self.strDispName = RecvMsg.MsgBody.OrigCalledName;
                break;
            default:
                break;
        }
        ...
        if (IDT.SRV_TYPE_CONF == self.SrvType || IDT.SRV_TYPE_CONF_JOIN == self.SrvType) {
            if (attr.VRx || attr.VTx) self.bIsGCall = false;else self.bIsGCall = true;
        } else {
            self.bIsGCall = false;
        }
        ...
        // 6.发送CALLIN到用户
        if (m_IDT_INST.CallBack.onCallInd) {
            m_IDT_INST.CallBack.onCallInd(IDT.CALL_EVENT_In, self.uiMyId, RecvMsg.MsgBody.TrueNum, RecvMsg.MsgBody.TrueName, self.SrvType, self.bIsGCall, attr.ARx, attr.ATx, attr.VRx, attr.VTx, RecvMsg.MsgBody.UserMark, RecvMsg.MsgBody.UserCallRef);
        }
    ...
    self.RecvConnAckProc = function (RecvMsg) {
        if (self.State != self.CC_WAIT_CONNACK) return 0;
        clearTimeout(self.TimerState);
        self.TimerState = null;
        self.State = self.CC_RUNNING;
        self.bConnected = true;
    ...
    self.RecvInfoProc = function (RecvMsg) {
        ...
        switch (RecvMsg.MsgBody.Info.Info) {
            case IDT.SRV_INFO_ICECAND:
                ...
                if (true == m_IDT_INST.bIsIe) {
                    if (null != self.idPeerV) self.idPeerV.addIceCandidate(newstr);
                } else {
                    self.PeerConn.addIceCandidate(cand).then(function () {
                        self.onAddIcePeerCandidateSuccess(cand);
                    }, function (err) {
                        self.onAddIcePeerCandidateError(cand, err);
                    });
                }
                break;
            ...
            case IDT.SRV_INFO_MICINFO:
                ...
                if (self.strTalkingNum != RecvMsg.MsgBody.UsrNum) {
                    ...
                    if (null == self.strTalkingNum) self.strTalkingNum = '';
                    if (null == self.strTalkingName) self.strTalkingName = '';
    ...
    self.RecvModifyProc = function (RecvMsg) {
        ...
        if (IDT.SRV_TYPE_CONF == self.SrvType || IDT.SRV_TYPE_CONF_JOIN == self.SrvType) {
            if (self.ucVRx || self.ucVTx) self.bIsGCall = false;else self.bIsGCall = true;
        } else {
            self.bIsGCall = false;
        }
        return 0;
    ...
    self.MicInd = function (bFirst) {
        if (false == self.bUserNotify) return 0;
        //没有接通过,不发送
        if (false == self.bConnected) return 0;

        let bInd = false;
        //第一次
        if (bFirst) {
            if (self.ucLastMic == self.ucATx) bInd = true;
        }
        //不是第一次
        else {
                if (2 == self.ucLastMic || self.ucLastMic != self.ucATx) bInd = true;
            }
